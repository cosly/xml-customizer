<script lang="ts">
  ...
  async function handleSubmit() {
    if (!formData.name.trim()) {
      error = $_('validation.required', { values: { field: $_('customers.customerName') } });
      return;
    }
...
<svelte:head>
  <title>{$_('customers.title')} - Tesoro</title>
</svelte:head>
      ...
      <form on:submit|preventDefault={handleSubmit}>
        <div class="modal-body">
          <div class="form-group">
            <label class="label" for="name">{$_('common.name')} *</label>
            <input
              class="input"
              type="text"
              id="name"
              bind:value={formData.name}
              placeholder={$_('customers.customerName')}
            />
          </div>
          <div class="form-group">
            <label class="label" for="email">{$_('customers.email')} ({$_('common.optional')})</label>
            <input
              class="input"
              type="email"
              id="email"
